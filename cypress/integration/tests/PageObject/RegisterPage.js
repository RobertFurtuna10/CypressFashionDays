...
class Register {
  ...
  VerifyEmptyEmailField() {
    cy.get(locators.emailErrorFieldMessage).should(
      "have.text",
      "Acest camp este obligatoriu"
    );
  }
  VerifyEmptyPasswordField() {
    cy.get(locators.passwordErrorFieldMessage).should(
      "have.text",
      "Acest camp este obligatoriu"
    );
  }
  VerifyEmptyCheckboxTerm() {
    cy.get(locators.checkboxTermErrorMessage).should(
      "have.text",
      "Te rugam sa accepti termenii si conditiile pentru a putea continua cu crearea contului"
    );
  }
  EnterEmail(email) {
  ...
  checkConfirmationText(confirmationText, genres) {
    return cy
      .mailosaurListMessages("oljudrdv")
      .then((result) => {
        if (result.items.length === 0) {
          throw new Error("No email received.");
        }
      ...
      .then((email) => {
        ...
        if (
          genres.toLowerCase() === "barbat" ||
          genres === "Sunt Barbat" ||
          genres === "man"
        ) {
          return cy.visit(confirmSignupLinkSuntBarbat);
        } else if (
          genres.toLowerCase() === "femeie" ||
          genres === "Sunt Femeie" ||
          genres === "women"
        ) {
